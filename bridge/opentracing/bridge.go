...
import (
	...
	"github.com/hanzoai/telemetry-go/bridge/opentracing/migration"
	"github.com/hanzoai/telemetry-go/codes"
	iBaggage "github.com/hanzoai/telemetry-go/internal/baggage"
...
func (t *BridgeTracer) StartSpan(operationName string, opts ...ot.StartSpanOption) ot.Span {
	...
	if ot.SpanFromContext(checkCtx2) != nil {
		t.warnOnce.Do(func() {
			t.warningHandler(
				"SDK should have deferred the context setup, see the documentation of github.com/hanzoai/telemetry-go/bridge/opentracing/migration\n",
			)
		})
	}
	if hadTrueErrorTag {
